...
var fieldsForCompany = [];
var fieldsForCopanyNames = [];

router.get('/company/:cik', function(req, res, next) {
  Company.find({ cik: req.params.cik}).populate('reviews').exec()
    .then(function(company) {
      json2csv({ data: company, fields: fieldsForCompany, fieldNames: fieldsForCompanyNames, quotes: '' }, function(err, csv) {
        res.send(csv);
      });
    }, function(err){
      res.send(err);
